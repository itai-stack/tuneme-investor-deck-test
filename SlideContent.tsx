    ...
    <div className={`flex-1 flex flex-col transition-all duration-1000 ${visibilityClass}`}>
      <div className="flex items-center justify-between mb-4 lg:mb-12 flex-shrink-0">
        <div className="flex items-center gap-2 lg:gap-3 text-[9px] lg:text-xs font-black tracking-[0.2em] uppercase text-white/60">
          <div className="w-1.5 h-1.5 lg:w-2 lg:h-2 rounded-full bg-teal-400" />
          {slide.kicker}
        </div>
        <div className="bg-slate-900/50 px-3 py-1.5 lg:px-4 lg:py-2 rounded-xl border border-white/10 custom-blur">
          <span className="text-teal-400 font-black text-xs lg:text-sm tracking-widest uppercase">TuneMe AI</span>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col justify-center min-h-0">
        <div className="max-w-5xl w-full">
          ...
          <div className="overflow-y-auto max-h-[50vh] lg:max-h-none pr-2 hide-scrollbar pb-10">
            {slide.kpis && (
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 lg:gap-4 mb-4 lg:mb-12">
                {slide.kpis.map((kpi, i) => (
                  <div key={i} className="bg-white/5 border border-white/10 p-2.5 lg:p-6 rounded-xl lg:rounded-2xl custom-blur group">
            ...
            {slide.pills && (
              <div className="flex flex-wrap gap-1.5 lg:gap-3">
                {slide.pills.map((pill, i) => (
                  <span key={i} className="px-2.5 lg:px-4 py-1 lg:py-2 rounded-full bg-white/5 border border-white/10 text-[8px] lg:text-xs font-bold tracking-wider text-white/70">
                    {pill}
                  </span>
                ))}
              </div>
            ...
            {slide.content && <div className="mt-4 lg:mt-8">{slide.content}</div>}
          </div>
        </div>
      </div>
    </div>
